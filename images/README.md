...
A virtualenv is created at `/root/.venv` during boot (using `uv venv`, ~100ms) with `--system-site-packages` so pre-installed packages from the rootfs are available immediately. The venv is activated in `clawcage-bashrc`. New `pip install` / `uv pip install` commands write to the venv on the scratch disk.
...
| Env Var | Source |
|---------|--------|
| `CLAWCAGE_ANTHROPIC_ALLOWED` | `ai.anthropic.allow` setting (1 or 0) |
| `CLAWCAGE_OPENAI_ALLOWED` | `ai.openai.allow` setting (1 or 0) |
| `CLAWCAGE_GOOGLE_ALLOWED` | `ai.google.allow` setting (1 or 0) |
...
The login banner (`clawcage-bashrc`) shows each tool's status:
...
- **`clawcage-bashrc`** -- Shell config that renders the banner, tip, and AI status section.
...
| `clawcage-init` | PID 1 init script (mounts, networking, agent launch) | `just run` |
| `clawcage-bashrc` | Guest shell config (venv, npm prefix, banner, AI status) | `just build-assets` |
...
| `clawcage-doctor` | In-VM diagnostic runner | `just build-assets` |
