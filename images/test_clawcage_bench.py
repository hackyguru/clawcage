"""Host-runnable unit tests for clawcage-bench.

Tests helper functions, table rendering (Rich markup safety), and benchmark
logic that doesn't require the VM. Run with: pytest images/test_clawcage_bench.py
"""
...
# ---------------------------------------------------------------------------
# Import clawcage-bench as a module (it has no .py extension)
# ---------------------------------------------------------------------------

def _import_clawcage_bench():
    bench_path = os.path.join(os.path.dirname(__file__), "clawcage-bench")
    loader = importlib.machinery.SourceFileLoader("clawcage_bench", bench_path)
    spec = importlib.util.spec_from_loader("clawcage_bench", loader,
                                           origin=bench_path)
