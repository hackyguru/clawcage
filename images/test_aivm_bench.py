"""Host-runnable unit tests for aivm-bench.

Tests helper functions, table rendering (Rich markup safety), and benchmark
logic that doesn't require the VM. Run with: pytest images/test_aivm_bench.py
"""
...
# ---------------------------------------------------------------------------
# Import aivm-bench as a module (it has no .py extension)
# ---------------------------------------------------------------------------

def _import_aivm_bench():
    bench_path = os.path.join(os.path.dirname(__file__), "aivm-bench")
    loader = importlib.machinery.SourceFileLoader("aivm_bench", bench_path)
    spec = importlib.util.spec_from_loader("aivm_bench", loader,
                                           origin=bench_path)
