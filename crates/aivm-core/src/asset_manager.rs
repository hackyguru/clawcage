...
pub struct AssetManager {
    /// Directory where downloaded assets are stored (~/.aivm/assets/).
    assets_dir: PathBuf,
...
impl AssetManager {
    ...
    ///
    /// `assets_dir` is where downloaded assets live (~/.aivm/assets/).
    /// `base_url` is the GitHub Releases download base, e.g.
    ///   `https://github.com/google/aivm/releases/download/v0.8.0`
...
/// Return the default assets directory: `~/.aivm/assets/`.
pub fn default_assets_dir() -> Option<PathBuf> {
    std::env::var("HOME")
        .ok()
        .map(|h| PathBuf::from(h).join(".aivm").join("assets"))
}

/// Build the GitHub Releases download base URL for the given version.
pub fn release_url(version: &str) -> String {
    format!(
        "https://github.com/google/aivm/releases/download/v{version}"
    )
}
...
";
    ...
    fn hash_file_known_content() {
        ...
        std::fs::write(&path, b"hello aivm").unwrap();

        let hash = hash_file(&path).unwrap();
        // Verify against blake3 crate directly.
        let expected = blake3::hash(b"hello aivm").to_hex().to_string();
    ...
    fn default_assets_dir_under_home() {
        ...
        assert!(dir.is_some());
        let d = dir.unwrap();
        assert!(d.to_string_lossy().contains(".aivm/assets"));
    }
