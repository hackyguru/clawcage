...
/// User config path: ~/.aivm/user.toml (overridable via AIVM_USER_CONFIG)
pub fn user_config_path() -> Option<std::path::PathBuf> {
    if let Ok(path) = std::env::var("AIVM_USER_CONFIG") {
        return Some(std::path::PathBuf::from(path));
    }
    dirs_path("HOME").map(|h| h.join(".aivm").join("user.toml"))
}

/// Corporate config path: /etc/aivm/corp.toml (overridable via AIVM_CORP_CONFIG)
pub fn corp_config_path() -> std::path::PathBuf {
    if let Ok(path) = std::env::var("AIVM_CORP_CONFIG") {
        return std::path::PathBuf::from(path);
    }
    std::path::PathBuf::from("/etc/aivm/corp.toml")
}
...
/// Venv config path: ~/.aivm/venvs/<venv_id>/settings.toml
pub fn venv_config_path(venv_id: &str) -> Option<std::path::PathBuf> {
    dirs_path("HOME").map(|h| h.join(".aivm").join("venvs").join(venv_id).join("settings.toml"))
}
...
/// Write user settings to ~/.aivm/user.toml.
...
/// Write venv settings to ~/.aivm/venvs/<venv_id>/settings.toml.
...
pub fn validate_setting_value(id: &str, value: &SettingValue) -> Result<(), String> {
    if let SettingValue::File { path, content } = value {
        // Validate path
        aivm_proto::validate_file_path(path)
...
pub fn settings_to_guest_config(resolved: &[ResolvedSetting]) -> GuestConfig {
    use aivm_proto::{validate_env_key, validate_env_value, validate_file_path};
    ...
    for s in resolved {
        ...
        // Provider allow toggles: inject AIVM_<PROVIDER>_ALLOWED=1|0
        // so the guest banner can show which AI tools are enabled.
        if s.setting_type == SettingType::Bool {
            let provider_env = match s.id.as_str() {
                "ai.anthropic.allow" => Some("AIVM_ANTHROPIC_ALLOWED"),
                "ai.openai.allow" => Some("AIVM_OPENAI_ALLOWED"),
                "ai.google.allow" => Some("AIVM_GOOGLE_ALLOWED"),
                _ => None,
            };
        ...
        if let SettingValue::File { path: file_path, content: file_content } = &s.effective_value {
            if !file_content.is_empty() {
                ...
                // Inject aivm MCP server into Claude/Gemini settings.json.
                ...
                let content = if file_path.ends_with("/settings.json") {
                    inject_aivm_mcp_server(file_content)
...
/// Inject the aivm MCP server entry into a settings.json string.
///
/// Parses the JSON, inserts `{"aivm": {"command": "/run/aivm-mcp-server"}}`
/// under `mcpServers`, preserving any user-provided entries. Returns the
/// original string unchanged if parsing fails.
fn inject_aivm_mcp_server(json_str: &str) -> String {
    ...
    if let Some(servers) = mcp_servers.as_object_mut() {
        servers.insert(
            "aivm".to_string(),
            serde_json::json!({"command": "/run/aivm-mcp-server"}),
        );
    }
...
"#;
    ...
    fn all_three_providers_injected() {
        ...
        // + 3 AIVM_*_ALLOWED provider flags
    ...
    fn provider_allowed_env_vars_injected() {
        // AIVM_*_ALLOWED env vars reflect the provider allow toggles.
        let user = file_with(vec![
            ("ai.anthropic.allow", SettingValue::Bool(true)),
            ("ai.openai.allow", SettingValue::Bool(false)),
            ("ai.google.allow", SettingValue::Bool(true)),
        ...
        let gc = settings_to_guest_config(&resolved);
        let env = gc.env.unwrap();
        assert_eq!(env.get("AIVM_ANTHROPIC_ALLOWED").unwrap(), "1");
        assert_eq!(env.get("AIVM_OPENAI_ALLOWED").unwrap(), "0");
        assert_eq!(env.get("AIVM_GOOGLE_ALLOWED").unwrap(), "1");
    }
    ...
    fn provider_allowed_defaults_to_zero() {
        ...
        let gc = settings_to_guest_config(&resolved);
        let env = gc.env.unwrap();
        assert_eq!(env.get("AIVM_ANTHROPIC_ALLOWED").unwrap(), "0");
        assert_eq!(env.get("AIVM_OPENAI_ALLOWED").unwrap(), "0");
        assert_eq!(env.get("AIVM_GOOGLE_ALLOWED").unwrap(), "1");
    }
    ...
    fn gemini_installation_id_is_file_type() {
        ...
        assert!(content.starts_with("aivm-sandbox-"));
    }
    ...
    fn each_boot_message_fits_in_frame() {
        ...
        use aivm_proto::{encode_host_msg, MAX_FRAME_SIZE};
        ...
        for (key, value) in gc.env.unwrap_or_default() {
            let msg = aivm_proto::HostToGuest::SetEnv { key: key.clone(), value: value.clone() };
        ...
        for f in gc.files.unwrap_or_default() {
            let msg = aivm_proto::HostToGuest::FileWrite {
                path: f.path.clone(),
    ...
    #[test]
    fn inject_aivm_mcp_server_into_empty_json() {
        let result = inject_aivm_mcp_server(r#"{}"#);
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap();
        assert_eq!(
            parsed["mcpServers"]["aivm"]["command"],
            "/run/aivm-mcp-server"
        );
    }

    #[test]
    fn inject_aivm_mcp_server_preserves_existing_servers() {
        let input = r#"{"mcpServers":{"github":{"command":"npx","args":["-y","@github/mcp"]}}}"#;
        let result = inject_aivm_mcp_server(input);
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap();
    ...
    #[test]
    fn inject_aivm_mcp_server_preserves_other_keys() {
    ...
    #[test]
    fn inject_aivm_mcp_server_invalid_json_passthrough() {
    ...
    #[test]
    fn claude_default_settings_has_aivm_mcp_server() {
        let resolved = resolve_settings(&empty_file(), &empty_file());
        let gc = settings_to_guest_config(&resolved);
        let files = gc.files.unwrap();
        ...
        assert_eq!(
            parsed["mcpServers"]["aivm"]["command"],
            "/run/aivm-mcp-server",
            "aivm MCP server should be injected into Claude settings.json"
        );
    ...
    #[test]
    fn gemini_default_settings_has_aivm_mcp_server() {
        let resolved = resolve_settings(&empty_file(), &empty_file());
        let gc = settings_to_guest_config(&resolved);
        let files = gc.files.unwrap();
        ...
        assert_eq!(
            parsed["mcpServers"]["aivm"]["command"],
            "/run/aivm-mcp-server",
            "aivm MCP server should be injected into Gemini settings.json"
        );
    }

    #[test]
    fn user_mcp_servers_preserved_alongside_aivm() {
    ...
    #[test]
    fn aivm_mcp_not_in_non_settings_json_files() {
        ...
        assert!(!projects.content.contains("aivm"), "projects.json should not have aivm injected");
    }
    ...
    fn venv_config_path_format() {
        ...
        assert_eq!(path, Some(std::path::PathBuf::from("/tmp/test_home/.aivm/venvs/my-venv-123/settings.toml")));
    }
