...
/// Re-exported so aivm-app can reference the type without depending on rustls.
...
async fn handle_inner(
    ...
    if initial_buf.starts_with(b"\0AIVM_META:") {
...
async fn handle_request(
    ...
    if is_upgrade {
        let body_text = format!(
            "Aivm: WebSocket upgrades are not supported ({} {})\n",
            method, path
        );
    ...
    let make_502 = |error: &dyn std::fmt::Display,
     ...
     -> hyper::Response<ProxyBoxBody> {
        ...
        let body_text = format!("Aivm: upstream error ({error})\n");
        let emitter = TelemetryEmitter {
            db: Arc::clone(db),
            config: Arc::clone(config),
            domain: domain.to_string(),
...
mod tests {
    ...
    const CA_KEY: &str = include_str!("../../../../config/aivm-ca.key");
    const CA_CERT: &str = include_str!("../../../../config/aivm-ca.crt");
    ...
    async fn fragmented_metadata_is_reassembled() {
        ...
        std::io::Write::write_all(&mut writer, b"\0AIVM_META:my_proc").unwrap();
    ...
    async fn oversized_metadata_rejected() {
        ...
        let mut oversized = b"\0AIVM_META:".to_vec();
