...
//! message per line) over the vsock, with a `\0AIVM_META:process_name\n`
//! metadata line first (same convention as aivm-net-proxy). No msgpack
...
use aivm_logger::{DbWriter, McpCall, WriteOp};
...
/// with a `\0AIVM_META:process_name\n` metadata line.
...
async fn serve_mcp_session_inner(fd: RawFd, config: &McpGatewayConfig) -> Result<()> {
    ...
    // Read the metadata line: \0AIVM_META:process_name\n
    ...
    let process_name = meta_line
        .strip_prefix('\0')
        .and_then(|s| s.strip_prefix("AIVM_META:"))
...
mod tests {
    ...
    fn handle_initialize() {
        ...
        assert_eq!(result["serverInfo"]["name"], "aivm-mcp-gateway");
    }
