...
pub struct McpResourceDef {
    /// Namespaced URI (e.g. "aivm://github/repo://owner/repo").
...
/// Create a namespaced resource URI: "aivm://github/repo://owner/repo"
pub fn namespace_resource_uri(server: &str, uri: &str) -> String {
    format!("aivm://{server}/{uri}")
}

/// Parse a namespaced resource URI back to (server, original_uri).
/// Input: "aivm://github/repo://owner/repo" -> ("github", "repo://owner/repo")
pub fn parse_resource_uri(namespaced: &str) -> Option<(&str, &str)> {
    let rest = namespaced.strip_prefix("aivm://")?;
...
mod tests {
    ...
    fn namespace_resource_uri_basic() {
        ...
        assert_eq!(uri, "aivm://github/repo://owner/repo");
    }

    #[test]
    fn parse_resource_uri_basic() {
        let (server, original) =
            parse_resource_uri("aivm://github/repo://owner/repo").unwrap();
        assert_eq!(server, "github");
    ...
    fn parse_resource_uri_nested_slashes() {
        let (server, original) =
            parse_resource_uri("aivm://fs/file:///home/user/doc.txt").unwrap();
        assert_eq!(server, "fs");
    ...
    fn parse_resource_uri_invalid() {
        ...
        assert!(parse_resource_uri("aivm://").is_none());
    }
