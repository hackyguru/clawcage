...
//!
//! Run with: AIVM_ASSETS_DIR=./assets cargo test --test vm_integration
//! They are skipped by default when assets are not present.

use std::path::PathBuf;

use aivm_core::VmConfig;

fn assets_dir() -> Option<PathBuf> {
    let dir = std::env::var("AIVM_ASSETS_DIR")
        .map(PathBuf::from)
...
fn vm_create_requires_entitlement() {
    ...
    let result = aivm_core::VirtualMachine::create(&config);
    match result {
...
fn vm_has_socket_devices_after_create() {
    ...
    match aivm_core::VirtualMachine::create(&config) {
        Ok((vm, _rx, _input_fd)) => {
...
fn vsock_manager_rejects_empty_devices() {
    ...
    let result = aivm_core::VsockManager::new(
        empty,
        &[aivm_core::VSOCK_PORT_CONTROL, aivm_core::VSOCK_PORT_TERMINAL],
    );
