//! Clawcage protocol types for host/guest communication over vsock.
//!
...
pub enum HostToGuest {
    ...
    /// Close a shell session in the guest.
...
pub enum GuestToHost {
    ...
    ShellReady { session_id: u32 },
    /// Shell session exited.
    ShellClosed { session_id: u32, exit_code: i32 },
    // -- Port telemetry --
    /// A TCP listening port was detected in the guest.
    PortOpened {
        port: u16,
        pid: u32,
        process: String,
...
// ---------------------------------------------------------------------------
// Terminal data framing for multiplexed sessions on vsock:5001
// ---------------------------------------------------------------------------

/// Session ID for the default (boot) shell.
pub const DEFAULT_SHELL_SESSION_ID: u32 = 0;

/// Encode terminal data with session framing: [4-byte BE len][4-byte BE session_id][data].
/// The length field covers session_id + data (so total frame = 4 + len).
pub fn encode_terminal_frame(session_id: u32, data: &[u8]) -> Vec<u8> {
...
/// Terminal frame header size: 4 (length) + 4 (session_id).
pub const TERMINAL_FRAME_HEADER_SIZE: usize = 8;
...
mod tests {
    ...
    fn all_host_variants_fit() {
        let messages = vec![
            ...
            HostToGuest::SpawnShell { session_id: u32::MAX },
            HostToGuest::CloseShell { session_id: u32::MAX },
            HostToGuest::ShellResize { session_id: u32::MAX, cols: u16::MAX, rows: u16::MAX },
    ...
    // -------------------------------------------------------------------
    // Multi-shell protocol
    // -------------------------------------------------------------------

    #[test]
    fn roundtrip_spawn_shell() {
        let msg = HostToGuest::SpawnShell { session_id: 1 };
        let frame = encode_host_msg(&msg).unwrap();
        let decoded = decode_host_msg(&frame[4..]).unwrap();
        match decoded {
            HostToGuest::SpawnShell { session_id } => assert_eq!(session_id, 1),
            other => panic!("expected SpawnShell, got {other:?}"),
        }
    }

    #[test]
    fn roundtrip_close_shell() {
        let msg = HostToGuest::CloseShell { session_id: 5 };
        let frame = encode_host_msg(&msg).unwrap();
        let decoded = decode_host_msg(&frame[4..]).unwrap();
        match decoded {
            HostToGuest::CloseShell { session_id } => assert_eq!(session_id, 5),
    ...
    fn roundtrip_shell_resize() {
        let msg = HostToGuest::ShellResize { session_id: 2, cols: 120, rows: 40 };
        let frame = encode_host_msg(&msg).unwrap();
        let decoded = decode_host_msg(&frame[4..]).unwrap();
        match decoded {
            HostToGuest::ShellResize { session_id, cols, rows } => {
    ...
    #[test]
    fn roundtrip_shell_ready() {
        let msg = GuestToHost::ShellReady { session_id: 3 };
        let frame = encode_guest_msg(&msg).unwrap();
        let decoded = decode_guest_msg(&frame[4..]).unwrap();
        match decoded {
            GuestToHost::ShellReady { session_id } => assert_eq!(session_id, 3),
            other => panic!("expected ShellReady, got {other:?}"),
        }
    }

    #[test]
    fn roundtrip_shell_closed() {
        let msg = GuestToHost::ShellClosed { session_id: 4, exit_code: 0 };
        let frame = encode_guest_msg(&msg).unwrap();
        let decoded = decode_guest_msg(&frame[4..]).unwrap();
        match decoded {
    ...
    #[test]
    fn roundtrip_shell_closed_negative_exit() {
        let msg = GuestToHost::ShellClosed { session_id: 1, exit_code: -1 };
        let frame = encode_guest_msg(&msg).unwrap();
        let decoded = decode_guest_msg(&frame[4..]).unwrap();
        match decoded {
            GuestToHost::ShellClosed { session_id, exit_code } => {
    ...
    // -------------------------------------------------------------------
    // Terminal data framing
    // -------------------------------------------------------------------

    #[test]
    fn terminal_frame_encoding() {
        let frame = encode_terminal_frame(1, b"hello");
        // [4-byte len][4-byte session_id][data]
        let len = u32::from_be_bytes([frame[0], frame[1], frame[2], frame[3]]);
        assert_eq!(len, 9); // 4 (session_id) + 5 (data)
        let session_id = u32::from_be_bytes([frame[4], frame[5], frame[6], frame[7]]);
        assert_eq!(session_id, 1);
        assert_eq!(&frame[8..], b"hello");
    }

    #[test]
    fn terminal_frame_default_session() {
        let frame = encode_terminal_frame(DEFAULT_SHELL_SESSION_ID, b"x");
        let session_id = u32::from_be_bytes([frame[4], frame[5], frame[6], frame[7]]);
    ...
    fn terminal_frame_empty_data() {
        let frame = encode_terminal_frame(42, b"");
        let len = u32::from_be_bytes([frame[0], frame[1], frame[2], frame[3]]);
        assert_eq!(len, 4); // just session_id, no data
        assert_eq!(frame.len(), 8); // header only
    }

    #[test]
    fn terminal_frame_header_size() {
        assert_eq!(TERMINAL_FRAME_HEADER_SIZE, 8);
    }
