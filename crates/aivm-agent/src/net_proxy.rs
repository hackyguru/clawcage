// aivm-net-proxy: Guest-side TCP-to-vsock relay for air-gapped networking.
//
...
async fn handle_connection(mut tcp_stream: TcpStream) {
    ...
    .await
    {
        Ok(Ok(fd)) => fd,
        _ => {
            eprintln!("[aivm-net-proxy] vsock connect failed");
            return;
        }
    };
    ...
    let mut vsock_stream = match AsyncVsock::new(vsock_raw) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[aivm-net-proxy] failed to create AsyncVsock: {e}");
            unsafe {
    ...
    let meta = format!("\0AIVM_META:{}\n", process_name);
    if let Err(e) = vsock_stream.write_all(meta.as_bytes()).await {
        eprintln!("[aivm-net-proxy] failed to inject process meta: {e}");
        return;
    }
    ...
    if let Err(e) = tokio::io::copy_bidirectional(&mut tcp_stream, &mut vsock_stream).await {
        ...
        if !is_normal {
            eprintln!("[aivm-net-proxy] bridge error: {e}");
        }
    }
}
...
async fn main() -> io::Result<()> {
    eprintln!("[aivm-net-proxy] starting (pid {})", process::id());

    let listener = TcpListener::bind(("127.0.0.1", LISTEN_PORT_HTTPS)).await?;
    eprintln!("[aivm-net-proxy] listening on 127.0.0.1:{LISTEN_PORT_HTTPS}");
    ...
    loop {
        tokio::select! {
            ...
            _ = signal::ctrl_c() => {
                eprintln!("[aivm-net-proxy] shutting down");
                break;
            }
        }
    }
