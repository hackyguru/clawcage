...
// aivm-fs-watch: In-VM inotify file watcher daemon.
//
    ...
    match encode_guest_msg(msg) {
        Ok(frame) => {
            if let Err(e) = write_all_fd(fd, &frame) {
                eprintln!("[aivm-fs-watch] write failed: {e}");
            }
        }
        Err(e) => {
            eprintln!("[aivm-fs-watch] encode failed: {e}");
        }
    }
}
...
fn run_watcher() {
    ...
    eprintln!("[aivm-fs-watch] starting (pid {})", std::process::id());
    ...
    fn add_watches_recursive(
        ...
        match inotify.add_watch(root, flags) {
            ...
            Err(e) => {
                eprintln!("[aivm-fs-watch] add_watch failed for {root}: {e}");
                return;
            }
        }
    ...
    eprintln!(
        "[aivm-fs-watch] watching {} directories under {WATCH_ROOT}",
        wd_to_path.len()
    );
    ...
    loop {
        if term_now.load(Ordering::Relaxed) {
            eprintln!("[aivm-fs-watch] received termination signal, flushing events");
        ...
        let events = match inotify.read_events() {
            ...
            Err(e) => {
                eprintln!("[aivm-fs-watch] read_events error: {e}");
...
fn run_watcher() {
    eprintln!("[aivm-fs-watch] inotify not available on this platform");
    std::process::exit(1);
}
