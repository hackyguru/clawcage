...
// Used by both aivm-pty-agent and aivm-net-proxy.
//
...
pub fn vsock_connect_retry(cid: u32, port: u32, label: &str) -> RawFd {
    ...
    loop {
        match vsock_connect(cid, port) {
            Ok(fd) => {
                eprintln!("[aivm-agent] {label} connected (port {port})");
                return fd;
            }
            Err(e) => {
                eprintln!("[aivm-agent] {label} connect failed: {e}, retrying in {delay_ms}ms");
