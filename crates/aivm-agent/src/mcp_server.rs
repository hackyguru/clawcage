// aivm-mcp-server: Guest-side MCP stdio-to-vsock relay.
//
...
//   2. Send metadata line: \0AIVM_META:process_name\n
...
fn main() {
    eprintln!("[aivm-mcp-server] starting (pid {})", process::id());
    ...
    if let Err(e) = write_all_fd(vsock_fd, meta.as_bytes()) {
        eprintln!("[aivm-mcp-server] failed to send metadata: {e}");
        process::exit(1);
    }
...
mod tests {
    ...
    fn meta_line_format() {
        ...
        assert!(meta.contains("AIVM_META:claude"));
