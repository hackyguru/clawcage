...
// aivm-sys-watch: In-VM system metrics daemon.
//
...
fn run_watcher() {
    ...
    eprintln!("[aivm-sys-watch] starting (pid {})", std::process::id());
    ...
    loop {
        if term_now.load(Ordering::Relaxed) {
            eprintln!("[aivm-sys-watch] received termination signal, exiting");
            break;
        }
        ...
        match encode_guest_msg(&msg) {
            Ok(frame) => {
                if let Err(e) = write_all_fd(vsock_fd, &frame) {
                    eprintln!("[aivm-sys-watch] write failed: {e}");
                    break;
                }
            }
            Err(e) => {
                eprintln!("[aivm-sys-watch] encode failed: {e}");
            }
        }
    }
}
...
fn run_watcher() {
    eprintln!("[aivm-sys-watch] /proc not available on this platform");
    std::process::exit(1);
}
