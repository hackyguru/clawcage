...
/// in TOML files at:
///   - User: ~/.clawcage/user.toml
///   - Corporate: /etc/clawcage/corp.toml
///
...
pub struct ResolvedSetting {
    ...
    pub enabled: bool,
    pub metadata: SettingMetadata,
    /// Whether this setting has a venv-level override (even if corp overrides it).
    #[serde(default)]
    pub venv_overridden: bool,
...
/// User config path: ~/.clawcage/user.toml (overridable via CLAWCAGE_USER_CONFIG)
pub fn user_config_path() -> Option<std::path::PathBuf> {
    if let Ok(path) = std::env::var("CLAWCAGE_USER_CONFIG") {
        return Some(std::path::PathBuf::from(path));
    }
    dirs_path("HOME").map(|h| h.join(".clawcage").join("user.toml"))
}

/// Corporate config path: /etc/clawcage/corp.toml (overridable via CLAWCAGE_CORP_CONFIG)
pub fn corp_config_path() -> std::path::PathBuf {
    if let Ok(path) = std::env::var("CLAWCAGE_CORP_CONFIG") {
        return std::path::PathBuf::from(path);
    }
    std::path::PathBuf::from("/etc/clawcage/corp.toml")
}
...
/// Venv config path: ~/.clawcage/venvs/<venv_id>/settings.toml
pub fn venv_config_path(venv_id: &str) -> Option<std::path::PathBuf> {
    dirs_path("HOME").map(|h| h.join(".clawcage").join("venvs").join(venv_id).join("settings.toml"))
}
...
/// Load all three settings files: user, venv (optional), and corp.
pub fn load_settings_files_with_venv(venv_id: Option<&str>) -> (SettingsFile, SettingsFile, SettingsFile) {
    let (user, corp) = load_settings_files();

    let venv = match venv_id {
        Some(id) => match venv_config_path(id) {
            Some(path) => load_settings_file(&path).unwrap_or_else(|e| {
                tracing::warn!("venv settings: {e}");
                SettingsFile::default()
            }),
            None => SettingsFile::default(),
        },
...
/// Write user settings to ~/.clawcage/user.toml.
...
/// Write venv settings to ~/.clawcage/venvs/<venv_id>/settings.toml.
pub fn write_venv_settings(venv_id: &str, file: &SettingsFile) -> Result<(), String> {
    let path = venv_config_path(venv_id).ok_or("HOME not set")?;
    write_settings_file(&path, file)
}

/// Remove a single setting from a venv's settings.toml (reset to inherit from user/default).
pub fn remove_venv_setting(venv_id: &str, setting_id: &str) -> Result<(), String> {
    let path = venv_config_path(venv_id).ok_or("HOME not set")?;
...
pub fn validate_setting_value(id: &str, value: &SettingValue) -> Result<(), String> {
    if let SettingValue::File { path, content } = value {
        // Validate path
        clawcage_proto::validate_file_path(path)
...
/// Resolve all settings from user + venv + corp files against the registry.
///
/// For each registered definition + any dynamic keys (guest.env.*),
/// corp overrides venv, venv overrides user, user overrides default.
/// Computes `enabled` from parent toggle.
pub fn resolve_settings(user: &SettingsFile, corp: &SettingsFile) -> Vec<ResolvedSetting> {
    resolve_settings_with_venv(user, &SettingsFile::default(), corp)
}

/// Resolve all settings with optional venv layer: corp > venv > user > default.
pub fn resolve_settings_with_venv(
    user: &SettingsFile,
    venv: &SettingsFile,
    corp: &SettingsFile,
) -> Vec<ResolvedSetting> {
    ...
    for def in &defs {
        let (effective_value, source, modified) = resolve_value_with_venv(&def.id, &def.default_value, user, venv, corp);
        let corp_locked = corp.settings.contains_key(&def.id);
        let venv_overridden = venv.settings.contains_key(&def.id);
    ...
    let dynamic_keys = collect_dynamic_keys_with_venv(user, venv, corp);
    for key in dynamic_keys {
        let default = SettingValue::Text(String::new());
        let (effective_value, source, modified) = resolve_value_with_venv(&key, &default, user, venv, corp);
        let corp_locked = corp.settings.contains_key(&key);
        let venv_overridden = venv.settings.contains_key(&key);
...
/// Resolve a single setting value: corp > venv > user > default.
fn resolve_value_with_venv(
    id: &str,
    default: &SettingValue,
    user: &SettingsFile,
    venv: &SettingsFile,
    ...
    } else if let Some(entry) = venv.settings.get(id) {
        (entry.value.clone(), PolicySource::Venv, Some(entry.modified.clone()))
...
/// Collect all dynamic keys (guest.env.*) from all three files.
fn collect_dynamic_keys_with_venv(user: &SettingsFile, venv: &SettingsFile, corp: &SettingsFile) -> Vec<String> {
    let mut keys: Vec<String> = user
        .settings
        .keys()
        .chain(venv.settings.keys())
...
pub fn settings_to_guest_config(resolved: &[ResolvedSetting]) -> GuestConfig {
    use clawcage_proto::{validate_env_key, validate_env_value, validate_file_path};
    ...
    for s in resolved {
        ...
        // Provider allow toggles: inject CLAWCAGE_<PROVIDER>_ALLOWED=1|0
        // so the guest banner can show which AI tools are enabled.
        if s.setting_type == SettingType::Bool {
            let provider_env = match s.id.as_str() {
                "ai.anthropic.allow" => Some("CLAWCAGE_ANTHROPIC_ALLOWED"),
                "ai.openai.allow" => Some("CLAWCAGE_OPENAI_ALLOWED"),
                "ai.google.allow" => Some("CLAWCAGE_GOOGLE_ALLOWED"),
                _ => None,
            };
        ...
        if let SettingValue::File { path: file_path, content: file_content } = &s.effective_value {
            if !file_content.is_empty() {
                ...
                // Inject clawcage MCP server into Claude/Gemini settings.json.
                ...
                let content = if file_path.ends_with("/settings.json") {
                    inject_clawcage_mcp_server(file_content)
...
/// Inject the clawcage MCP server entry into a settings.json string.
///
/// Parses the JSON, inserts `{"clawcage": {"command": "/run/clawcage-mcp-server"}}`
/// under `mcpServers`, preserving any user-provided entries. Returns the
/// original string unchanged if parsing fails.
fn inject_clawcage_mcp_server(json_str: &str) -> String {
    ...
    if let Some(servers) = mcp_servers.as_object_mut() {
        servers.insert(
            "clawcage".to_string(),
            serde_json::json!({"command": "/run/clawcage-mcp-server"}),
        );
    }
...
pub fn load_merged_policy() -> HttpPolicy {
    load_merged_policy_for_venv(None)
}

/// Load and merge settings for a specific venv, then build an HttpPolicy.
pub fn load_merged_policy_for_venv(venv_id: Option<&str>) -> HttpPolicy {
    let (user, venv, corp) = load_settings_files_with_venv(venv_id);
    let resolved = resolve_settings_with_venv(&user, &venv, &corp);
...
pub fn load_merged_domain_policy() -> DomainPolicy {
    load_merged_domain_policy_for_venv(None)
}

/// Build a `DomainPolicy` for a specific venv.
pub fn load_merged_domain_policy_for_venv(venv_id: Option<&str>) -> DomainPolicy {
    let (user, venv, corp) = load_settings_files_with_venv(venv_id);
    let resolved = resolve_settings_with_venv(&user, &venv, &corp);
...
pub fn load_merged_network_policy() -> super::policy::NetworkPolicy {
    load_merged_network_policy_for_venv(None)
}

/// Build a `NetworkPolicy` for a specific venv.
pub fn load_merged_network_policy_for_venv(venv_id: Option<&str>) -> super::policy::NetworkPolicy {
...
pub fn load_merged_guest_config() -> GuestConfig {
    load_merged_guest_config_for_venv(None)
}

/// Load and merge guest config for a specific venv.
pub fn load_merged_guest_config_for_venv(venv_id: Option<&str>) -> GuestConfig {
    let (user, venv, corp) = load_settings_files_with_venv(venv_id);
    let resolved = resolve_settings_with_venv(&user, &venv, &corp);
...
pub fn load_merged_vm_settings() -> VmSettings {
    load_merged_vm_settings_for_venv(None)
}

/// Load and merge VM settings for a specific venv.
pub fn load_merged_vm_settings_for_venv(venv_id: Option<&str>) -> VmSettings {
    let (user, venv, corp) = load_settings_files_with_venv(venv_id);
    let resolved = resolve_settings_with_venv(&user, &venv, &corp);
...
pub fn load_merged_settings() -> Vec<ResolvedSetting> {
    load_merged_settings_for_venv(None)
}

/// Load all resolved settings for a specific venv.
pub fn load_merged_settings_for_venv(venv_id: Option<&str>) -> Vec<ResolvedSetting> {
    let (user, venv, corp) = load_settings_files_with_venv(venv_id);
    resolve_settings_with_venv(&user, &venv, &corp)
}
...
pub fn load_merged_lint() -> Vec<ConfigIssue> {
    load_merged_lint_for_venv(None)
}

/// Run lint for a specific venv's merged settings.
pub fn load_merged_lint_for_venv(venv_id: Option<&str>) -> Vec<ConfigIssue> {
    let (user, venv, corp) = load_settings_files_with_venv(venv_id);
    let resolved = resolve_settings_with_venv(&user, &venv, &corp);
...
pub fn load_settings_tree() -> Vec<SettingsNode> {
    load_settings_tree_for_venv(None)
}

/// Load settings tree for a specific venv.
pub fn load_settings_tree_for_venv(venv_id: Option<&str>) -> Vec<SettingsNode> {
    let (user, venv, corp) = load_settings_files_with_venv(venv_id);
    let resolved = resolve_settings_with_venv(&user, &venv, &corp);
...
"#;
    ...
    fn all_three_providers_injected() {
        ...
        // + 3 CLAWCAGE_*_ALLOWED provider flags
    ...
    fn provider_allowed_env_vars_injected() {
        // CLAWCAGE_*_ALLOWED env vars reflect the provider allow toggles.
        let user = file_with(vec![
            ("ai.anthropic.allow", SettingValue::Bool(true)),
            ("ai.openai.allow", SettingValue::Bool(false)),
            ("ai.google.allow", SettingValue::Bool(true)),
        ...
        let gc = settings_to_guest_config(&resolved);
        let env = gc.env.unwrap();
        assert_eq!(env.get("CLAWCAGE_ANTHROPIC_ALLOWED").unwrap(), "1");
        assert_eq!(env.get("CLAWCAGE_OPENAI_ALLOWED").unwrap(), "0");
        assert_eq!(env.get("CLAWCAGE_GOOGLE_ALLOWED").unwrap(), "1");
    }
    ...
    fn provider_allowed_defaults_to_zero() {
        ...
        let gc = settings_to_guest_config(&resolved);
        let env = gc.env.unwrap();
        assert_eq!(env.get("CLAWCAGE_ANTHROPIC_ALLOWED").unwrap(), "0");
        assert_eq!(env.get("CLAWCAGE_OPENAI_ALLOWED").unwrap(), "0");
        assert_eq!(env.get("CLAWCAGE_GOOGLE_ALLOWED").unwrap(), "1");
    }
    ...
    fn gemini_installation_id_is_file_type() {
        ...
        assert!(content.starts_with("clawcage-sandbox-"));
    }
    ...
    fn each_boot_message_fits_in_frame() {
        ...
        use clawcage_proto::{encode_host_msg, MAX_FRAME_SIZE};
        ...
        for (key, value) in gc.env.unwrap_or_default() {
            let msg = clawcage_proto::HostToGuest::SetEnv { key: key.clone(), value: value.clone() };
        ...
        for f in gc.files.unwrap_or_default() {
            let msg = clawcage_proto::HostToGuest::FileWrite {
                path: f.path.clone(),
    ...
    #[test]
    fn inject_clawcage_mcp_server_into_empty_json() {
        let result = inject_clawcage_mcp_server(r#"{}"#);
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap();
        assert_eq!(
            parsed["mcpServers"]["clawcage"]["command"],
            "/run/clawcage-mcp-server"
        );
    }

    #[test]
    fn inject_clawcage_mcp_server_preserves_existing_servers() {
        let input = r#"{"mcpServers":{"github":{"command":"npx","args":["-y","@github/mcp"]}}}"#;
        let result = inject_clawcage_mcp_server(input);
        let parsed: serde_json::Value = serde_json::from_str(&result).unwrap();
    ...
    #[test]
    fn inject_clawcage_mcp_server_preserves_other_keys() {
    ...
    #[test]
    fn inject_clawcage_mcp_server_invalid_json_passthrough() {
    ...
    #[test]
    fn claude_default_settings_has_clawcage_mcp_server() {
        let resolved = resolve_settings(&empty_file(), &empty_file());
        let gc = settings_to_guest_config(&resolved);
        let files = gc.files.unwrap();
        ...
        assert_eq!(
            parsed["mcpServers"]["clawcage"]["command"],
            "/run/clawcage-mcp-server",
            "clawcage MCP server should be injected into Claude settings.json"
        );
    ...
    #[test]
    fn gemini_default_settings_has_clawcage_mcp_server() {
        let resolved = resolve_settings(&empty_file(), &empty_file());
        let gc = settings_to_guest_config(&resolved);
        let files = gc.files.unwrap();
        ...
        assert_eq!(
            parsed["mcpServers"]["clawcage"]["command"],
            "/run/clawcage-mcp-server",
            "clawcage MCP server should be injected into Gemini settings.json"
        );
    }

    #[test]
    fn user_mcp_servers_preserved_alongside_clawcage() {
    ...
    #[test]
    fn clawcage_mcp_not_in_non_settings_json_files() {
        ...
        assert!(!projects.content.contains("clawcage"), "projects.json should not have clawcage injected");
    }
    ...
    #[test]
    fn venv_override_takes_precedence_over_user() {
        let user = file_with(vec![("ai.anthropic.allow", SettingValue::Bool(false))]);
        let venv = file_with(vec![("ai.anthropic.allow", SettingValue::Bool(true))]);
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
        let s = resolved.iter().find(|s| s.id == "ai.anthropic.allow").unwrap();
        assert_eq!(s.effective_value, SettingValue::Bool(true));
        assert_eq!(s.source, PolicySource::Venv);
        assert!(s.venv_overridden);
    }

    #[test]
    fn corp_overrides_venv() {
        let user = file_with(vec![("ai.anthropic.allow", SettingValue::Bool(true))]);
        let venv = file_with(vec![("ai.anthropic.allow", SettingValue::Bool(true))]);
        let corp = file_with(vec![("ai.anthropic.allow", SettingValue::Bool(false))]);
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
        ...
        assert_eq!(s.effective_value, SettingValue::Bool(false));
        assert_eq!(s.source, PolicySource::Corp);
        // Venv has an override, even though corp wins
        assert!(s.venv_overridden);
    }

    #[test]
    fn venv_overridden_false_when_no_venv_entry() {
        let user = file_with(vec![("ai.anthropic.allow", SettingValue::Bool(true))]);
        let venv = empty_file();
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
    ...
    #[test]
    fn venv_override_text_setting() {
        let user = file_with(vec![("network.default_action", SettingValue::Text("deny".into()))]);
        let venv = file_with(vec![("network.default_action", SettingValue::Text("allow".into()))]);
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
        let s = resolved.iter().find(|s| s.id == "network.default_action").unwrap();
        assert_eq!(s.effective_value, SettingValue::Text("allow".into()));
        assert_eq!(s.source, PolicySource::Venv);
    }

    #[test]
    fn venv_override_number_setting() {
        let user = file_with(vec![("vm.max_body_capture", SettingValue::Number(4096))]);
        let venv = file_with(vec![("vm.max_body_capture", SettingValue::Number(8192))]);
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
        let s = resolved.iter().find(|s| s.id == "vm.max_body_capture").unwrap();
        assert_eq!(s.effective_value, SettingValue::Number(8192));
        assert_eq!(s.source, PolicySource::Venv);
    }

    #[test]
    fn venv_override_dynamic_env() {
        let user = file_with(vec![("guest.env.EDITOR", SettingValue::Text("vim".into()))]);
        let venv = file_with(vec![("guest.env.EDITOR", SettingValue::Text("nano".into()))]);
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
        let s = resolved.iter().find(|s| s.id == "guest.env.EDITOR").unwrap();
        assert_eq!(s.effective_value, SettingValue::Text("nano".into()));
    ...
    #[test]
    fn venv_only_dynamic_env_key_appears() {
        let user = empty_file();
        let venv = file_with(vec![("guest.env.MY_VAR", SettingValue::Text("hello".into()))]);
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
        let s = resolved.iter().find(|s| s.id == "guest.env.MY_VAR").unwrap();
        assert_eq!(s.effective_value, SettingValue::Text("hello".into()));
        assert_eq!(s.source, PolicySource::Venv);
    }

    #[test]
    fn empty_venv_falls_through_to_user() {
        let user = file_with(vec![("ai.anthropic.allow", SettingValue::Bool(true))]);
        let venv = empty_file();
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
    ...
    #[test]
    fn empty_venv_falls_through_to_default() {
        let user = empty_file();
        let venv = empty_file();
        let corp = empty_file();
        let resolved = resolve_settings_with_venv(&user, &venv, &corp);
    ...
    #[test]
    fn venv_config_path_format() {
        // Verify the path format without hitting the filesystem
        std::env::set_var("HOME", "/tmp/test_home");
        let path = venv_config_path("my-venv-123");
        assert_eq!(path, Some(std::path::PathBuf::from("/tmp/test_home/.clawcage/venvs/my-venv-123/settings.toml")));
    }

    #[test]
    fn resolve_settings_backward_compat() {
        // The 2-param resolve_settings should behave identically to the
        // 3-param version with an empty venv file (no regressions).
        let user = file_with(vec![
            ("ai.anthropic.allow", SettingValue::Bool(true)),
            ("network.default_action", SettingValue::Text("allow".into())),
        ]);
        ...
        let old_resolved = resolve_settings(&user, &corp);
        let new_resolved = resolve_settings_with_venv(&user, &empty_file(), &corp);
        // Same number of settings
        assert_eq!(old_resolved.len(), new_resolved.len());
        // Same values for all settings
        for (old, new) in old_resolved.iter().zip(new_resolved.iter()) {
            assert_eq!(old.id, new.id);
            assert_eq!(old.effective_value, new.effective_value);
            assert_eq!(old.source, new.source);
            assert!(!new.venv_overridden, "no venv file => no venv_overridden");
        }
    }
}
