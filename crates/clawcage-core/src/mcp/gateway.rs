...
//! message per line) over the vsock, with a `\0CLAWCAGE_META:process_name\n`
//! metadata line first (same convention as clawcage-net-proxy). No msgpack
...
use clawcage_logger::{DbWriter, McpCall, WriteOp};
...
/// with a `\0CLAWCAGE_META:process_name\n` metadata line.
...
async fn serve_mcp_session_inner(fd: RawFd, config: &McpGatewayConfig) -> Result<()> {
    ...
    // Read the metadata line: \0CLAWCAGE_META:process_name\n
    ...
    let process_name = meta_line
        .strip_prefix('\0')
        .and_then(|s| s.strip_prefix("CLAWCAGE_META:"))
...
mod tests {
    ...
    fn handle_initialize() {
        ...
        assert_eq!(result["serverInfo"]["name"], "clawcage-mcp-gateway");
    }
    ...
    #[test]
    fn parse_meta_line() {
        let meta = "\0CLAWCAGE_META:claude\n";
        let name = meta
            .strip_prefix('\0')
            .and_then(|s| s.strip_prefix("CLAWCAGE_META:"))
            .map(|s| s.trim().to_string())
