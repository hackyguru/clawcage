...
pub struct McpResourceDef {
    /// Namespaced URI (e.g. "clawcage://github/repo://owner/repo").
...
/// Create a namespaced resource URI: "clawcage://github/repo://owner/repo"
pub fn namespace_resource_uri(server: &str, uri: &str) -> String {
    format!("clawcage://{server}/{uri}")
}

/// Parse a namespaced resource URI back to (server, original_uri).
/// Input: "clawcage://github/repo://owner/repo" -> ("github", "repo://owner/repo")
pub fn parse_resource_uri(namespaced: &str) -> Option<(&str, &str)> {
    let rest = namespaced.strip_prefix("clawcage://")?;
...
mod tests {
    ...
    fn namespace_resource_uri_basic() {
        ...
        assert_eq!(uri, "clawcage://github/repo://owner/repo");
    }

    #[test]
    fn parse_resource_uri_basic() {
        let (server, original) =
            parse_resource_uri("clawcage://github/repo://owner/repo").unwrap();
        assert_eq!(server, "github");
    ...
    fn parse_resource_uri_nested_slashes() {
        let (server, original) =
            parse_resource_uri("clawcage://fs/file:///home/user/doc.txt").unwrap();
        assert_eq!(server, "fs");
    ...
    fn parse_resource_uri_invalid() {
        ...
        assert!(parse_resource_uri("clawcage://").is_none());
    }
