...
//!
//! Run with: CLAWCAGE_ASSETS_DIR=./assets cargo test --test vm_integration
//! They are skipped by default when assets are not present.

use std::path::PathBuf;

use clawcage_core::VmConfig;

fn assets_dir() -> Option<PathBuf> {
    let dir = std::env::var("CLAWCAGE_ASSETS_DIR")
        .map(PathBuf::from)
...
fn vm_create_requires_entitlement() {
    ...
    let result = clawcage_core::VirtualMachine::create(&config);
    match result {
...
fn vm_has_socket_devices_after_create() {
    ...
    match clawcage_core::VirtualMachine::create(&config) {
        Ok((vm, _rx, _input_fd)) => {
...
fn vsock_manager_rejects_empty_devices() {
    ...
    let result = clawcage_core::VsockManager::new(
        empty,
        &[clawcage_core::VSOCK_PORT_CONTROL, clawcage_core::VSOCK_PORT_TERMINAL],
    );
