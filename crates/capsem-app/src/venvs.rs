...
//!
//! Stores venv metadata in `~/.capsem/venvs.json`. Each venv is a named
...
/// Returns the path to `~/.capsem/venvs.json`.
fn venvs_path() -> Result<PathBuf, String> {
    let home = std::env::var("HOME").map_err(|_| "HOME not set".to_string())?;
    let dir = PathBuf::from(home).join(".capsem");
    ...
    tokio::task::spawn_blocking(move || {
        ...
        if let Ok(home) = std::env::var("HOME") {
            let venv_dir = std::path::PathBuf::from(home)
                .join(".capsem").join("venvs").join(&id);
            if venv_dir.exists() {
