// clawcage-net-proxy: Guest-side TCP-to-vsock relay for air-gapped networking.
//
...
async fn handle_connection(mut tcp_stream: TcpStream) {
    ...
    let mut vsock_stream = match AsyncVsock::new(vsock_raw) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[clawcage-net-proxy] failed to create AsyncVsock: {e}");
            unsafe {
    ...
    let meta = format!("\0CLAWCAGE_META:{}\n", process_name);
    if let Err(e) = vsock_stream.write_all(meta.as_bytes()).await {
        eprintln!("[clawcage-net-proxy] failed to inject process meta: {e}");
        return;
    }
    ...
    if let Err(e) = tokio::io::copy_bidirectional(&mut tcp_stream, &mut vsock_stream).await {
        ...
        if !is_normal {
            eprintln!("[clawcage-net-proxy] bridge error: {e}");
        }
    }
}
...
async fn main() -> io::Result<()> {
    ...
    loop {
        tokio::select! {
            ...
            _ = signal::ctrl_c() => {
                eprintln!("[clawcage-net-proxy] shutting down");
                break;
            }
        }
    }
