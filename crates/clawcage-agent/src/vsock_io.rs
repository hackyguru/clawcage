...
// Used by both clawcage-pty-agent and clawcage-net-proxy.
//
...
pub fn vsock_connect_retry(cid: u32, port: u32, label: &str) -> RawFd {
    ...
    loop {
        match vsock_connect(cid, port) {
            Ok(fd) => {
                eprintln!("[clawcage-agent] {label} connected (port {port})");
                return fd;
            }
            Err(e) => {
                eprintln!("[clawcage-agent] {label} connect failed: {e}, retrying in {delay_ms}ms");
