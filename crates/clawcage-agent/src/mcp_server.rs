// clawcage-mcp-server: Guest-side MCP stdio-to-vsock relay.
//
...
fn main() {
    ...
    if let Err(e) = write_all_fd(vsock_fd, meta.as_bytes()) {
        eprintln!("[clawcage-mcp-server] failed to send metadata: {e}");
        process::exit(1);
    }
...
mod tests {
    ...
    fn meta_line_format() {
        ...
        assert!(meta.contains("CLAWCAGE_META:claude"));
