// clawcage-pty-agent: Guest-side multi-shell PTY-over-vsock bridge.
//
// Runs inside the Linux VM as a child of clawcage-init. Manages multiple PTY
...
/// Boot log persisted so it can be inspected after boot (`cat /var/log/clawcage-boot.log`).
const BOOT_LOG_PATH: &str = "/var/log/clawcage-boot.log";

// ---------------------------------------------------------------------------
// Control message framing (using clawcage-proto types)
// ---------------------------------------------------------------------------
...
fn set_system_clock(epoch_secs: u64) {
    ...
    if ret == 0 {
        eprintln!("[clawcage-agent] clock set to epoch {epoch_secs}");
    } else {
        eprintln!(
            "[clawcage-agent] WARNING: clock_settime failed ({}): \
...
// ---------------------------------------------------------------------------
// Boot log -- persists at /var/log/clawcage-boot.log for post-boot diagnosis
// ---------------------------------------------------------------------------
...
fn main() {
    ...
    if let Err(e) = send_guest_msg(control_fd, &GuestToHost::Ready {
        ...
        eprintln!("[clawcage-agent] failed to send Ready: {e}");
        process::exit(1);
    }
    ...
    match recv_host_msg(control_fd) {
        Ok(HostToGuest::BootConfig { epoch_secs }) => {
            eprintln!("[clawcage-agent] received BootConfig (epoch={epoch_secs})");
        ...
        Ok(other) => {
            ...
            eprintln!("[clawcage-agent] expected BootConfig, got {other:?}, continuing with defaults");
        }
        Err(e) => {
            blog_line(&mut blog, &format!("BootConfig error: {e}"));
            eprintln!("[clawcage-agent] failed to receive BootConfig: {e}, continuing with defaults");
        }
    };
    ...
    loop {
        match recv_host_msg(control_fd) {
            Ok(HostToGuest::SetEnv { key, value }) => {
                ...
                eprintln!("[clawcage-agent] SetEnv {key}");
            ...
            Ok(HostToGuest::FileWrite { path, data, mode }) => {
                ...
                if let Some(parent) = std::path::Path::new(&path).parent() {
                    if let Err(e) = std::fs::create_dir_all(parent) {
                        ...
                        eprintln!("[clawcage-agent] failed to create dir {}: {e}", parent.display());
                        continue;
                    }
                }
                if let Err(e) = std::fs::write(&path, &data) {
                    blog_line(&mut blog, &format!("FileWrite {path}: write failed: {e}"));
                    eprintln!("[clawcage-agent] failed to write {path}: {e}");
                    continue;
                }
                #[cfg(unix)]
                {
                ...
                eprintln!("[clawcage-agent] wrote {path} ({} bytes)", data.len());
            }
            Ok(HostToGuest::BootConfigDone) => {
                ...
                eprintln!("[clawcage-agent] boot config done ({} env vars, {} files)", boot_env.len(), file_count);
                break;
            }
            Ok(other) => {
                blog_line(&mut blog, &format!("unexpected boot message: {other:?}"));
                eprintln!("[clawcage-agent] unexpected message during boot: {other:?}");
            }
            Err(e) => {
                blog_line(&mut blog, &format!("boot handshake error: {e}"));
                eprintln!("[clawcage-agent] boot handshake error: {e}, proceeding with what we have");
                break;
            }
        }
    }
    ...
    if let Err(e) = send_guest_msg(control_fd, &GuestToHost::BootReady) {
        eprintln!("[clawcage-agent] failed to send BootReady: {e}");
    }
...
/// Format: ESC _ CLAWCAGE_EXIT:{id}:{exit_code} ESC \
const SENTINEL_PREFIX: &[u8] = b"\x1b_CLAWCAGE_EXIT:";
...
fn run_multi_bridge(
    ...
    eprintln!("[clawcage-agent] bridge exited, killing all shells");
...
///
/// The sentinel format is: ESC _ CLAWCAGE_EXIT:{id}:{exit_code} ESC \
...
fn vsock_to_pty_loop(vsock_fd: RawFd, shell_mgr: &ShellManager) {
    ...
    loop {
        ...
        if len < 4 {
            eprintln!("[clawcage-agent] invalid terminal frame len={len}");
            break;
        }
        let data_len = len - 4;
        if data_len == 0 {
...
fn multi_bridge_loop(
    ...
    loop {
        ...
        match poll(&mut poll_fds, PollTimeout::from(200u16)) {
            ...
            Err(e) => {
                eprintln!("[clawcage-agent] poll error: {e}");
                break;
            }
        }
        ...
        for session_id in closed_sessions {
            if let Some(shell) = shell_mgr.remove(session_id) {
                ...
                let _ = nix::unistd::close(shell.master_fd);
                eprintln!("[clawcage-agent] shell {session_id} closed (exit_code={exit_code})");
        ...
        match recv_host_msg(control_fd) {
            Ok(HostToGuest::Resize { cols, rows }) => {
                ...
                if let Some(master_fd) = shell_mgr.get_master_fd(DEFAULT_SHELL_SESSION_ID) {
                    eprintln!("[clawcage-agent] resize(0): {cols}x{rows}");
                    set_winsize(master_fd, cols, rows);
                    unsafe {
            ...
            Ok(HostToGuest::ShellResize { session_id, cols, rows }) => {
                if let Some(master_fd) = shell_mgr.get_master_fd(session_id) {
                    eprintln!("[clawcage-agent] resize({session_id}): {cols}x{rows}");
                    set_winsize(master_fd, cols, rows);
                    unsafe {
                        let mut pgrp: libc::pid_t = 0;
                        if libc::ioctl(master_fd, libc::TIOCGPGRP, &mut pgrp) == 0 && pgrp > 0 {
                    ...
                    eprintln!("[clawcage-agent] resize for unknown session {session_id}");
                }
            }
            Ok(HostToGuest::SpawnShell { session_id }) => {
                eprintln!("[clawcage-agent] SpawnShell({session_id})");
                if shell_mgr.count() >= MAX_SHELLS {
                    eprintln!("[clawcage-agent] shell cap reached ({MAX_SHELLS})");
                ...
                if shell_mgr.get_master_fd(session_id).is_some() {
                    eprintln!("[clawcage-agent] session {session_id} already exists");
                    continue;
                }
                match spawn_shell(session_id, 80, 24, &boot_env) {
                    Ok(shell) => {
                        ...
                        if let Err(e) = send_guest_msg(control_fd, &GuestToHost::ShellReady { session_id }) {
                            eprintln!("[clawcage-agent] failed to send ShellReady: {e}");
                        }
                    }
                    Err(e) => {
                        eprintln!("[clawcage-agent] failed to spawn shell {session_id}: {e}");
                        let _ = send_guest_msg(control_fd, &GuestToHost::ShellClosed {
                            session_id,
                            exit_code: -1,
                        });
            ...
            Ok(HostToGuest::CloseShell { session_id }) => {
                eprintln!("[clawcage-agent] CloseShell({session_id})");
                if let Some(shell) = shell_mgr.remove(session_id) {
                    let _ = nix::sys::signal::kill(shell.child_pid, Signal::SIGHUP);
                    ...
                    if let Err(e) = send_guest_msg(control_fd, &GuestToHost::ShellClosed {
                        ...
                        eprintln!("[clawcage-agent] failed to send ShellClosed: {e}");
                    }
                }
            }
            Ok(HostToGuest::Ping) => {
                if let Err(e) = send_guest_msg(control_fd, &GuestToHost::Pong) {
                    eprintln!("[clawcage-agent] failed to send Pong: {e}");
                    break;
                }
            }
            Ok(HostToGuest::FileRead { id, path }) => {
                eprintln!("[clawcage-agent] FileRead[{id}]: {path}");
                ...
                match std::fs::metadata(&path) {
                    ...
                    Ok(meta) => {
                        ...
                        eprintln!("[clawcage-agent] FileRead[{id}]: sending {} bytes in chunks", total_size);
                            ...
                            Ok(mut file) => {
                                ...
                                loop {
                                    ...
                                    if send_guest_msg(control_fd, &GuestToHost::FileChunk {
                                        ...
                                        eprintln!("[clawcage-agent] FileRead[{id}]: failed to send chunk at offset {offset}");
                                        failed = true;
                                        break;
                                    }
                                    offset += n as u64;
            ...
            Ok(HostToGuest::FileSave { id, path, data }) => {
                eprintln!("[clawcage-agent] FileSave[{id}]: {path} ({} bytes)", data.len());
                if let Err(e) = clawcage_proto::validate_file_path(&path) {
                    let _ = send_guest_msg(control_fd, &GuestToHost::FileError {
                        id,
                        error: format!("invalid path: {e}"),
            ...
            Ok(HostToGuest::Exec { id, command }) => {
                ...
                let master_fd = match shell_mgr.get_master_fd(DEFAULT_SHELL_SESSION_ID) {
                    Some(fd) => fd,
                    None => {
                        eprintln!("[clawcage-agent] exec[{id}]: no default shell");
                ...
                eprintln!("[clawcage-agent] exec[{id}]: {command}");
                {
                ...
                let injection = format!(
                    "bash -c '{}' ; printf '\\033_CLAWCAGE_EXIT:{}:%d\\033\\\\' $?\n",
                    command.replace('\'', "'\\''"),
                    id,
                );
                if let Err(e) = write_all_fd(master_fd, injection.as_bytes()) {
                    eprintln!("[clawcage-agent] failed to inject exec command: {e}");
                    exec_state.active.store(false, Ordering::Release);
                    let _ = send_guest_msg(control_fd, &GuestToHost::ExecDone {
                        id,
                        exit_code: 126,
                ...
                match exec_done_rx.recv() {
                    Ok((done_id, exit_code)) => {
                        eprintln!("[clawcage-agent] exec[{done_id}] done: exit_code={exit_code}");
                        unsafe {
                            let mut termios: libc::termios = std::mem::zeroed();
                            libc::tcgetattr(master_fd, &mut termios);
                            termios.c_lflag |= libc::ECHO;
                        ...
                        if let Err(e) = send_guest_msg(control_fd, &GuestToHost::ExecDone {
                            ...
                            eprintln!("[clawcage-agent] failed to send ExecDone: {e}");
                            break;
                        }
                    }
                    Err(_) => {
                        eprintln!("[clawcage-agent] exec_done channel closed");
                        break;
                    }
                }
            }
            Ok(msg) => {
                eprintln!("[clawcage-agent] unhandled control message: {msg:?}");
            }
            Err(e) => {
                eprintln!("[clawcage-agent] control channel error: {e}");
                break;
            }
        }
    }
...
mod tests {
    ...
    fn agent_ready_roundtrip() {
        ...
        let decoded: GuestToHost = clawcage_proto::decode_guest_msg(&payload).unwrap();
        match decoded {
    ...
    fn host_resize_decodable_by_agent() {
        ...
        let frame = clawcage_proto::encode_host_msg(&msg).unwrap();
    ...
    fn boot_ready_roundtrip_over_pipe() {
        ...
        let mut payload = vec![0u8; len];
        read_exact_fd(read_fd, &mut payload).unwrap();
        let decoded = clawcage_proto::decode_guest_msg(&payload).unwrap();
    ...
    fn send_recv_multiple_messages_over_pipe() {
        ...
        // Send host messages.
        let ping_frame = clawcage_proto::encode_host_msg(&HostToGuest::Ping).unwrap();
        write_all_fd(write_fd, &ping_frame).unwrap();
        let resize_frame = clawcage_proto::encode_host_msg(&HostToGuest::Resize { cols: 80, rows: 24 }).unwrap();
    ...
    fn sentinel_detected_in_single_chunk() {
        ...
        let data = b"some output\x1b_CLAWCAGE_EXIT:42:0\x1b\\more data";
    ...
    fn sentinel_with_nonzero_exit_code() {
        let mut tail = Vec::new();
        let data = b"error output\x1b_CLAWCAGE_EXIT:7:127\x1b\\";
        let (forward, result) = scan_and_strip_sentinel(&mut tail, data);
    ...
    fn sentinel_negative_exit_code() {
        let mut tail = Vec::new();
        let data = b"\x1b_CLAWCAGE_EXIT:1:-1\x1b\\";
        let (forward, result) = scan_and_strip_sentinel(&mut tail, data);
        assert!(forward.is_empty());
