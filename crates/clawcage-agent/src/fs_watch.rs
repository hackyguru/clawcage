...
// clawcage-fs-watch: In-VM inotify file watcher daemon.
//
    ...
    match encode_guest_msg(msg) {
        Ok(frame) => {
            if let Err(e) = write_all_fd(fd, &frame) {
                eprintln!("[clawcage-fs-watch] write failed: {e}");
            }
        }
        Err(e) => {
            eprintln!("[clawcage-fs-watch] encode failed: {e}");
        }
    }
}
...
fn run_watcher() {
    ...
    fn add_watches_recursive(
        ...
        match inotify.add_watch(root, flags) {
            ...
            Err(e) => {
                eprintln!("[clawcage-fs-watch] add_watch failed for {root}: {e}");
                return;
            }
        }
    ...
    eprintln!(
        "[clawcage-fs-watch] watching {} directories under {WATCH_ROOT}",
        wd_to_path.len()
    );
    ...
    if init_count > 0 {
        eprintln!("[clawcage-fs-watch] initial scan: {init_count} existing files");
    }
    ...
    loop {
        if term_now.load(Ordering::Relaxed) {
            eprintln!("[clawcage-fs-watch] received termination signal, flushing events");
        ...
        let events = match inotify.read_events() {
            ...
            Err(e) => {
                eprintln!("[clawcage-fs-watch] read_events error: {e}");
...
fn run_watcher() {
    eprintln!("[clawcage-fs-watch] inotify not available on this platform");
    std::process::exit(1);
}
