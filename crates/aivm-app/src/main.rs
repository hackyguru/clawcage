...
use aivm_core::mcp::gateway::{self, McpGatewayConfig};
...
fn resolve_assets_dir() -> Result<PathBuf> {
    ...
    Err(anyhow::anyhow!(
        "VM assets not found. Set AIVM_ASSETS_DIR or run from workspace root."
    ))
}

/// Resolve rootfs path, checking bundled assets first, then ~/.aivm/assets/.
...
/// Write boot performance data from the state machine to ~/.aivm/perf/<timestamp>.log
fn write_perf_log(sm: &HostStateMachine) {
    ...
    let dir = home.join(".aivm").join("perf");
...
/// Get the sessions base directory: ~/.aivm/sessions/
fn sessions_dir() -> Option<PathBuf> {
    std::env::var("HOME").ok().map(|h| {
        PathBuf::from(h).join(".aivm").join("sessions")
    })
}

/// Get the session directory for a specific VM: ~/.aivm/sessions/<vm_id>/
...
fn cleanup_stale_sessions(index: &SessionIndex) {
    ...
    if let Ok(sessions) = index.recent(1000) {
        for rec in &sessions {
            ...
            if let Ok(reader) = aivm_logger::DbReader::open(&db_path) {
...
fn flush_session_summary(
    session_id: &str,
    index: &SessionIndex,
    reader: &aivm_logger::DbReader,
) {
    use aivm_core::session::{McpToolSummary, ProviderSummary, ToolSummary};
...
const CA_KEY_PEM: &str = include_str!("../../../config/aivm-ca.key");
const CA_CERT_PEM: &str = include_str!("../../../config/aivm-ca.crt");
...
fn boot_vm(
    ...
    let config = {
        ...
        // Use explicit rootfs override if provided (e.g. from ~/.aivm/assets/),
...
async fn serial_to_events(
    ...
    let queue = terminal_output.get_or_create(aivm_core::aivm_proto::DEFAULT_SHELL_SESSION_ID);
    loop {
        match rx.recv().await {
            Ok(bytes) => {
                queue.push(bytes);
...
async fn vsock_terminal_to_events(
    ...
    std::thread::spawn(move || {
        ...
        let mut header = [0u8; aivm_core::aivm_proto::TERMINAL_FRAME_HEADER_SIZE];
        loop {
...
async fn handle_fs_watch(fd: RawFd, db: Arc<DbWriter>) {
    use aivm_logger::{FileAction, FileEvent, WriteOp};
...
fn parse_env_args(args: &[String]) -> (Vec<(String, String)>, Vec<String>) {
    use aivm_core::aivm_proto::{validate_env_key, validate_env_value};
        ...
        if arg == "--env" {
            if let Some(val) = iter.next() {
                if let Some((key, value)) = val.split_once('=') {
                    if let Err(e) = validate_env_key(key) {
                        eprintln!("aivm: --env rejected: {e}");
                        continue;
                    }
                    if let Err(e) = validate_env_value(value) {
                        eprintln!("aivm: --env {key} rejected: {e}");
                        continue;
                    }
                    env_pairs.push((key.to_string(), value.to_string()));
                } else {
                    eprintln!("aivm: --env value must be KEY=VALUE, got: {val}");
                }
            } else {
                eprintln!("aivm: --env requires a KEY=VALUE argument");
            }
            ...
            if let Some((key, value)) = rest.split_once('=') {
                ...
                eprintln!("aivm: --env value must be KEY=VALUE, got: {rest}");
            }
        } else {
...
fn run_cli(command: &str, cli_env: &[(String, String)], session_index: &SessionIndex) -> Result<()> {
    ...
    // Resolve rootfs: check bundled assets first, then ~/.aivm/assets/.
    // If missing, download it before booting.
    let rootfs_path = match resolve_rootfs(&assets) {
        Some(path) => Some(path),
        None => {
            eprintln!("[aivm] rootfs not found, downloading...");
            ...
            let downloaded = rt.block_on(mgr.download_asset(&name, &client, |p| {
                if p.total_bytes > 0 {
                    ...
                    eprint!("\r[aivm] {}: {} bytes   ", p.phase, p.bytes_downloaded);
                }
            }))?;
            eprintln!();
            Some(downloaded)
    ...
    eprintln!("[aivm] session: {cli_session_id}");
    ...
    while terminal_fd.is_none() || control_fd.is_none() {
        ...
        if !warned_setup && setup_start.elapsed() > Duration::from_secs(30) {
            eprintln!("[aivm] warning: no vsock connections after 30s. Is the guest agent running?");
            warned_setup = true;
        }
    ...
    loop {
        ...
        match ctrl_msg_rx.try_recv() {
            Ok(GuestToHost::Ready { version }) => {
                eprintln!("[aivm] guest agent ready (v{version})");
                break;
            }
            Ok(other) => {
                eprintln!("[aivm] unexpected control message before Ready: {other:?}");
            }
    ...
    loop {
        if Instant::now() >= boot_ready_deadline {
            eprintln!("[aivm] BootReady not received within 5s, proceeding");
            break;
        }
        unsafe {
            core_foundation_sys::runloop::CFRunLoopRunInMode(
        ...
        match ctrl_msg_rx.try_recv() {
            Ok(GuestToHost::BootReady) => {
                eprintln!("[aivm] guest boot ready");
                break;
            }
            Ok(other) => {
                eprintln!("[aivm] control message during boot: {other:?}");
            }
            Err(std::sync::mpsc::TryRecvError::Empty) => continue,
            Err(std::sync::mpsc::TryRecvError::Disconnected) => {
    ...
    let terminal_reader = std::thread::spawn(move || {
        let mut file = match clone_fd(terminal_fd) {
            Ok(f) => f,
            Err(e) => {
                eprintln!("[aivm] terminal reader failed to clone fd: {e}");
                return;
            }
        };
    ...
    loop {
        if Instant::now() >= deadline {
            eprintln!("[aivm] timed out waiting for command completion");
            exit_code = 124; // Same as `timeout` command.
            break;
        }
        if !warned_exec && last_msg_time.elapsed() > Duration::from_secs(30) {
            eprintln!("[aivm] warning: no control messages (heartbeats) for 30s. Guest may be hung.");
            warned_exec = true;
        }
        unsafe {
            core_foundation_sys::runloop::CFRunLoopRunInMode(
        ...
        match ctrl_msg_rx.try_recv() {
            ...
            Ok(other) => {
                last_msg_time = Instant::now();
                warned_exec = false;
                eprintln!("[aivm] control message during exec: {other:?}");
            }
            Err(std::sync::mpsc::TryRecvError::Empty) => continue,
            Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                eprintln!("[aivm] control channel closed during exec");
                exit_code = 1;
                break;
            }
        }
...
async fn check_for_update(app: tauri::AppHandle) {
    ...
    let accepted = app
        .dialog()
        .message(format!(
            "Aivm {} is available (you have {}). Download and install?",
...
/// Per-venv data directory: ~/.aivm/venvs/<venv_id>/
fn venv_scratch_dir(venv_id: &str) -> Option<PathBuf> {
    std::env::var("HOME").ok().map(|h| {
        PathBuf::from(h).join(".aivm").join("venvs").join(venv_id)
    })
}
    ...
    if let Some(instance) = instance {
        ...
        // /root may be lost because the VM stop is abrupt.
        {
            ...
            if let Ok(mut file) = clone_fd(term_fd) {
                ...
                let payload = b"\x03\nsync\n";
                let frame = aivm_core::aivm_proto::encode_terminal_frame(
                    aivm_core::aivm_proto::DEFAULT_SHELL_SESSION_ID,
                    payload,
                );
...
pub(crate) fn boot_venv(handle: &tauri::AppHandle, venv_id: &str) -> Result<(), String> {
    ...
    // Per-venv scratch disk: ~/.aivm/venvs/<venv_id>/scratch.img
...
fn main() {
    ...
    let filter = match std::env::var("RUST_LOG") {
        ...
        Err(_) => {
            ...
            EnvFilter::new(format!("aivm={level},aivm_core={level}"))
        }
    };
    ...
    let session_index = match sessions_dir() {
        Some(d) => {
            ...
            match SessionIndex::open(&d.join("main.db")) {
                Ok(idx) => idx,
                Err(e) => {
                    eprintln!("aivm: failed to open session index: {e}");
                    std::process::exit(1);
                }
            }
        }
        None => {
            eprintln!("aivm: HOME not set, cannot create session index");
            std::process::exit(1);
        }
    };
    ...
    if !cli_args.is_empty() {
        ...
        if remaining_args.is_empty() {
            eprintln!("aivm: no command specified");
            std::process::exit(1);
        }
