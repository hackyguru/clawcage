...
use aivm_core::net::policy_config::{self, ConfigIssue, ResolvedSetting, SettingEntry, SettingsNode, SettingValue};
use aivm_core::session;
use aivm_core::{HostToGuest, encode_host_msg, validate_host_msg};
use aivm_logger::validate_select_only;
use serde::Serialize;
use tauri::State;
...
pub async fn serial_input(
    ...
    let sid = session_id.unwrap_or(aivm_core::aivm_proto::DEFAULT_SHELL_SESSION_ID);
...
pub async fn terminal_resize(
    ...
    let msg = if sid == aivm_core::aivm_proto::DEFAULT_SHELL_SESSION_ID {
...
pub async fn get_network_policy() -> Result<NetworkPolicyResponse, String> {
    tokio::task::spawn_blocking(|| {
        ...
        let (default_act, _) = dp.evaluate("__aivm_probe_nonexistent__.invalid");
        ...
        NetworkPolicyResponse {
            allow: allow_set,
            block: block_set,
            default_action: if default_act == aivm_core::net::domain_policy::Action::Allow {
                "allow".to_string()
            } else {
                "deny".to_string()
            },
...
/// Set a guest env var in ~/.aivm/user.toml via settings system.
#[tauri::command]
...
/// Remove a guest env var from ~/.aivm/user.toml via settings system.
#[tauri::command]
...
mod tests {
    use super::*;
    use crate::state::AppState;
    use aivm_core::session::SessionIndex;
