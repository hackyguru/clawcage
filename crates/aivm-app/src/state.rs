...
use aivm_core::mcp::gateway::McpGatewayConfig;
use aivm_core::net::cert_authority::CertAuthority;
use aivm_core::net::policy::NetworkPolicy;
use aivm_core::session::SessionIndex;
use aivm_core::aivm_proto::DEFAULT_SHELL_SESSION_ID;
...
pub struct VmNetworkState {
    ...
    pub upstream_tls: Arc<aivm_core::net::mitm_proxy::UpstreamTlsConfig>,
}
        ...
        std::thread::spawn(move || {
            ...
            while let Ok((fd, session_id, data)) = rx.recv() {
                ...
                if let Some(mut file) = current_file.as_ref() {
                    ...
                    let frame = aivm_core::aivm_proto::encode_terminal_frame(session_id, &buf);
