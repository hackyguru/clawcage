# Clawcage Toolchain Skill

How to build, test, and ship Clawcage. All workflows use `just` (not make).
...
| Smoke | `just run "clawcage-doctor"` | Repack + boot VM + run diagnostic suite | Yes |
| Full | `just full-test` | `test` + clawcage-doctor + integration test + bench | Yes (3x) |
...
- **`just run "clawcage-doctor"`**: Sandbox integrity, network isolation, MITM trust chain, runtime availability, MCP gateway
...
Both gate on `full-test` passing first. `just release` produces `target/release/Clawcage.dmg`. `just install` copies the `.app` to `/Applications` and launches it.
...
Only needed when changing `Dockerfile.rootfs`, `clawcage-bashrc`, `diagnostics/`, installed packages, or kernel config. Guest binary changes (clawcage-init, clawcage-agent, clawcage-net-proxy, clawcage-mcp-server, clawcage-fs-watch) are handled by `just run` via initrd repack.
...
| Changed clawcage-init | `just run` |
