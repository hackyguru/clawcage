...
Use this skill when debugging clawcage issues, verifying telemetry pipelines, or validating guest VM behavior.
...
3. **Run in-VM diagnostics**: `just run "clawcage-doctor"`
...
- Guest: is `clawcage-fs-watch` running? Check boot logs for `[clawcage-fs-watch] starting`
- Host: is the vsock accept for port 5005 succeeding? Check for `[clawcage-agent] fs-watch connected`
...
```bash
just run "clawcage-doctor"              # Full suite
just run "clawcage-doctor -k sandbox"   # Only sandbox tests
just run "clawcage-doctor -x"           # Stop on first failure
```
...
1. **Run the integration test** to generate a rich session:
   ```bash
   python3 scripts/integration_test.py --binary target/debug/clawcage --assets assets
   ```
...
3. **Verify the session has everything the fixture tests need**:
   ```bash
   sqlite3 ~/.clawcage/sessions/<id>/session.db "
...
4. **Update the fixture**:
   ```bash
   just update-fixture ~/.clawcage/sessions/<id>/session.db
   ```
...
1. Check boot logs -- did the relevant daemon start? (`clawcage-fs-watch`, `clawcage-net-proxy`, etc.)
...
4. Check the session DB directly: `sqlite3 ~/.clawcage/sessions/<id>/session.db "SELECT * FROM fs_events"`

### "Guest binary not picking up changes"

- Changed `clawcage-init`, agent, or repacked binary? -> `just run` (auto-repacks initrd)
- Changed rootfs (Dockerfile, bashrc, diagnostics)? -> `just build-assets`
- Binary on rootfs vs initrd: initrd copies take priority (clawcage-init checks `/binary` before rootfs path)
...
- Run `cargo build --release --target aarch64-unknown-linux-musl -p clawcage-agent` to isolate the failure
