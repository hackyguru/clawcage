...
First-launch experience for new Clawcage users. Depends on the distribution foundation (asset manager, thin DMG, update story) being complete.

## Scope

### 1. Welcome Screen
- What Clawcage is and what it does
...
- Run clawcage-doctor subset (sandbox integrity, network isolation)
...
- Wizard state stored in `~/.clawcage/onboarding.json` (tracks completed steps)
...
- `clawcage --setup` CLI flag to re-run the wizard
