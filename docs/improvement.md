...
- **Strategy (Host):** Enabled host-level caching (`VZDiskImageCachingMode::Cached`) and disabled strict synchronization barriers (`VZDiskImageSynchronizationMode::None`) for both rootfs and ephemeral scratch disks in `crates/clawcage-core/src/vm/machine.rs`.
- **Strategy (Guest):** Added `sysfs` tuning to `images/clawcage-init` setting the I/O scheduler to `none` (delegating to macOS) and increasing `read_ahead_kb` to `4096` and `nr_requests` to `256` for all VirtIO block devices. Added `noatime` and `nodiratime` to ext4 mount options.
...
- **Async Implementation:** Replaced the synchronous, thread-per-connection `net-proxy` with a Tokio-based async implementation in `clawcage-agent`. This resolves potential deadlocks and improves concurrency handling.
