...
export default function DetailPanel({ selection, onClose }: Props) {
  ...
  const renderThinking = () => (
    <div className="whitespace-pre-wrap text-sm font-mono text-content/80 leading-relaxed">
  ...
  const renderTool = () => (
    <div className="space-y-3">
      <div>
        <div className="text-xs text-content/50 mb-1">Tool Name</div>
        <div className="font-mono text-sm">{str(data.tool_name)}</div>
      </div>
      {!!data.arguments && (
        <div>
          <div className="text-xs text-content/50 mb-1">Arguments</div>
          <pre
            className="text-xs font-mono bg-surface-alt rounded p-2 overflow-auto max-h-60"
      ...
      {!!data.response_preview && (
        <div>
          <div className="text-xs text-content/50 mb-1">Response</div>
          <pre className="text-xs font-mono bg-surface-alt rounded p-2 overflow-auto max-h-60">
            {str(data.response_preview)}
          </pre>
        </div>
      )}
      {data.is_error ? (
        <div className="inline-flex items-center px-2 py-0.5 text-xs rounded-full bg-denied/15 text-denied">Error</div>
      ) : null}
    </div>
  );
  ...
  const renderNetEvent = () => (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div>
          <div className="text-xs text-content/50">Domain</div>
          <div className="font-mono">{str(data.domain)}</div>
        </div>
        <div>
          <div className="text-xs text-content/50">Method</div>
          <div className="font-mono">{str(data.method) || '-'}</div>
        </div>
        <div>
          <div className="text-xs text-content/50">Status</div>
          <div className="font-mono">{str(data.status_code) || '-'}</div>
        </div>
        <div>
          <div className="text-xs text-content/50">Decision</div>
          <div className={`font-mono ${data.decision === 'allowed' ? 'text-allowed' : 'text-denied'}`}>
            {str(data.decision)}
          </div>
        </div>
        <div>
          <div className="text-xs text-content/50">Path</div>
          <div className="font-mono truncate">{str(data.path) || '/'}</div>
        </div>
        <div>
          <div className="text-xs text-content/50">Duration</div>
          <div className="font-mono">{data.duration_ms ? `${data.duration_ms}ms` : '-'}</div>
        </div>
      </div>
      {!!data.request_body_preview && (
        <div>
          <div className="text-xs text-content/50 mb-1">Request Body</div>
          <pre className="text-xs font-mono bg-surface-alt rounded p-2 overflow-auto max-h-40">
      ...
      {!!data.response_body_preview && (
        <div>
          <div className="text-xs text-content/50 mb-1">Response Body</div>
          <pre className="text-xs font-mono bg-surface-alt rounded p-2 overflow-auto max-h-40">
  ...
  const renderMcpCall = () => (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div>
          <div className="text-xs text-content/50">Server</div>
          <div className="font-mono">{str(data.server_name)}</div>
        </div>
        <div>
          <div className="text-xs text-content/50">Tool</div>
  ...
  const renderFileEvent = () => (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div>
          <div className="text-xs text-content/50">Action</div>
        ...
        {data.size != null && (
          <div>
            <div className="text-xs text-content/50">Size</div>
